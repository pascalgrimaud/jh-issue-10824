...
/**
 * Integration tests for the {@link TestServiceImplResource} REST controller.
 */
@SpringBootTest(classes = TravisNg2App.class)
public class TestServiceImplResourceIT {
     ...
     * if they test an entity which requires the current entity.
     */
    public static TestServiceImpl createUpdatedEntity(EntityManager em) {
        TestServiceImpl testServiceImpl = new TestServiceImpl();
        return testServiceImpl;
    }
    ...
    @Test
    @Transactional
    public void getTestServiceImplsByIdFiltering() throws Exception {
        // Initialize the database
        testServiceImplRepository.saveAndFlush(testServiceImpl);

        Long id = testServiceImpl.getId();

        defaultTestServiceImplShouldBeFound("id.equals=" + id);
        defaultTestServiceImplShouldNotBeFound("id.notEquals=" + id);

        defaultTestServiceImplShouldBeFound("id.greaterThanOrEqual=" + id);
        defaultTestServiceImplShouldNotBeFound("id.greaterThan=" + id);

        defaultTestServiceImplShouldBeFound("id.lessThanOrEqual=" + id);
        defaultTestServiceImplShouldNotBeFound("id.lessThan=" + id);
    }
