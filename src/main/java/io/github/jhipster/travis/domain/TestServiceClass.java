...
public class TestServiceClass implements Serializable {
    ...
    @ManyToOne
    @JsonIgnoreProperties("testServiceClasses")
    private User userOneToMany;

    @ManyToMany
    @Cache(usage = CacheConcurrencyStrategy.NONSTRICT_READ_WRITE)
    @JoinTable(name = "test_service_class_user_many_to_many",
               joinColumns = @JoinColumn(name = "test_service_class_id", referencedColumnName = "id"),
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof TestServiceClass)) {
            return false;
        }
        return id != null && id.equals(((TestServiceClass) o).id);
    }
