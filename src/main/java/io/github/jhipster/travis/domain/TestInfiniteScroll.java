...
public class TestInfiniteScroll implements Serializable {
    ...
    @ManyToOne
    @JsonIgnoreProperties("testInfiniteScrolls")
    private User userOneToMany;

    @ManyToMany
    @Cache(usage = CacheConcurrencyStrategy.NONSTRICT_READ_WRITE)
    @JoinTable(name = "test_infinite_scroll_user_many_to_many",
               joinColumns = @JoinColumn(name = "test_infinite_scroll_id", referencedColumnName = "id"),
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof TestInfiniteScroll)) {
            return false;
        }
        return id != null && id.equals(((TestInfiniteScroll) o).id);
    }
