...
public class TestMapstruct implements Serializable {
    ...
    @ManyToOne
    @JsonIgnoreProperties("testMapstructs")
    private User userOneToMany;

    @ManyToMany
    @Cache(usage = CacheConcurrencyStrategy.NONSTRICT_READ_WRITE)
    @JoinTable(name = "test_mapstruct_user_many_to_many",
               joinColumns = @JoinColumn(name = "test_mapstruct_id", referencedColumnName = "id"),
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof TestMapstruct)) {
            return false;
        }
        return id != null && id.equals(((TestMapstruct) o).id);
    }
